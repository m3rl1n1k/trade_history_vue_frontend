...
const save = async () => {
  ...
  try {
    let response;
    if (!props.isEdit) {
      response = await axios.post("https://51.20.114.2/api/v1/categories/sub/new", data, {
        headers: {
      ...
      response = await axios.put(`https://51.20.114.2/api/v1/categories/sub/edit/${props.subCategory.id}`, data, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${sessionStorage.getItem('token')}`
        }
...
const deleteSubCategory = async () => {
  isLoading.value = true
  try {
    const response = await axios.delete(`https://51.20.114.2/api/v1/categories/sub/delete/${props.subCategory.id}`, {
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem('token')}`
      }
    })
